"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GalleryService = void 0;
const common_1 = require("@nestjs/common");
const gallery_entity_1 = require("../entities/gallery.entity");
const typeorm_1 = require("typeorm");
const typeorm_2 = require("@nestjs/typeorm");
let GalleryService = class GalleryService {
    constructor(galleryEntityRepository) {
        this.galleryEntityRepository = galleryEntityRepository;
    }
    async create(createGalleryDto) {
        try {
            const gallery = await this.galleryEntityRepository.save(createGalleryDto);
            return gallery;
        }
        catch (error) {
            throw new common_1.HttpException(error.message, error.status);
        }
    }
    async findAll(query) {
        console.log('query', query);
        try {
            if ((query === null || query === void 0 ? void 0 : query.passportNo) || (query === null || query === void 0 ? void 0 : query.regDate)) {
                const gallery = await this.galleryEntityRepository.find({
                    where: query,
                });
                return gallery;
            }
            else {
                throw new common_1.HttpException({ status: common_1.HttpStatus.FORBIDDEN, message: `Data not found` }, common_1.HttpStatus.FORBIDDEN);
            }
        }
        catch (error) {
            throw new common_1.HttpException(error.message, error.status);
        }
    }
    async findOne(passportNo) {
        try {
            const gallery = await this.galleryEntityRepository.findOne({
                where: { passportNo: passportNo },
            });
            if (gallery) {
                throw new common_1.HttpException({ status: common_1.HttpStatus.FORBIDDEN, message: `photo is already exsist!` }, common_1.HttpStatus.FORBIDDEN);
            }
            return gallery;
        }
        catch (error) {
            throw new common_1.HttpException(error.message, error.status);
        }
    }
    update(id, updateGalleryDto) {
        return `This action updates a #${id} gallery`;
    }
    remove(id) {
        return `This action removes a #${id} gallery`;
    }
};
GalleryService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_2.InjectRepository)(gallery_entity_1.GalleryEntity)),
    __metadata("design:paramtypes", [typeorm_1.Repository])
], GalleryService);
exports.GalleryService = GalleryService;
//# sourceMappingURL=gallery.service.js.map